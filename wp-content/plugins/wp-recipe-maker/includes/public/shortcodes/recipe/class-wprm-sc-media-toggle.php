...
class WPRM_SC_Media_Toggle extends WPRM_Template_Shortcode {
	...
	public static function shortcode( $atts ) {
		...
		foreach ( $buttons as $index => $button ) {
			...
			$active = 0 === $index ? ' wprm-toggle-active' : '';
			$buttons_output .= '<a href="#" class="wprm-recipe-media-toggle wprm-toggle' . $active . '" data-state="' . $button . '" data-recipe="' . esc_attr( $recipe->id() ) . '" style="' . $button_style .'">' . $button_text . '</a>';
		}
